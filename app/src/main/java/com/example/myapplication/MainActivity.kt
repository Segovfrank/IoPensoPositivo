...
class MainActivity : AppCompatActivity() {
    ...
    companion object {
        ...
        private val mensajesPositivos = listOf(
    ...
    private fun initViews() {
        ...
        indexButton = findViewById(R.id.btn_index)
        ...
        indexButton.setOnClickListener {
            if(!textInput.text.isNullOrEmpty()){
                val pos = textInput.text.toString().toInt()
                if(pos in mensajesPositivos.indices){
                    textLayout.error = null
                    textView.text = mensajesPositivos[pos]
                }else{
                    Toast.makeText(this, "Error con el numero", Toast.LENGTH_SHORT).show()
    ...
    private fun checkPosition(){
        if(currentPosition == mensajesPositivos.size){
            currentPosition = 0
        }
        if(currentPosition < 0){
            currentPosition = mensajesPositivos.size - 1
        }
